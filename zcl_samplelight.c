...
//-- MOD END
  ...
  if ( zclSampleLight_LevelRemainingTime == 0)
  {
    ...
    if (zclSampleLight_LevelLastReportValue > zclSampleLight_LevelCurrentLevel) {
        if (zclSampleLight_LevelLastReportValue - zclSampleLight_LevelCurrentLevel >= zclSampleLight_LevelCurrentLevelChange) {
            needReport = TRUE;
        }
    } else {
        if (zclSampleLight_LevelCurrentLevel - zclSampleLight_LevelLastReportValue >= zclSampleLight_LevelCurrentLevelChange) {
            needReport = TRUE;
        }
    }

    if (needReport) {
        sendZclAttrChangeReport(SAMPLELIGHT_ENDPOINT, ZCL_CLUSTER_ID_GEN_LEVEL_CONTROL, ATTRID_LEVEL_CURRENT_LEVEL,
        ...
        {   
            if (pConfigReportRec->timeup == 0xFFFF || pConfigReportRec->timeup == gTimeCounter) // timeup to report
            {
               // zclSampleLight_OnOffCB(COMMAND_ON);
                ...
                numAttr++;
                zclFindAttrRec( SAMPLELIGHT_ENDPOINT, pConfigReportRec->clusterId, pConfigReportRec->cfgReportRec.attrID, &attrRec);
 ...
 *          clusterID - cluster id
 *          attrID - attribute id
 *          lastReportValue - pointer to the last reported attribute value
 *          currentValue - pointer to the current reported attribute value
 *
 * @return  none
 */
