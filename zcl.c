 ...
 * @return  pointer to record list, NULL if not found
 */
// remove static modifier, since we need this function out of zcl.c
 ...
 *
 * @brief   Find the configReportRec record that matchs the parameters
 *
 * @param   endpoint - Application's endpoint
 * @param   clusterID - cluster ID
 * @param   attrId - attribute looking for
...
uint8 zclFindConfigReportRec( uint8 endpoint, uint16 clusterID, uint16 attrId, zclConfigReportRec_t **pConfigReportRec )
{
  ...
  if ( pRec != NULL )
  {
    ...
    {
      if ( pRec->configReportRecs[x].clusterId == clusterID && pRec->configReportRecs[x].cfgReportRec.attrID == attrId )
      {
        *pConfigReportRec = &(pRec->configReportRecs[x]);    // return the address, since we can modify it by ZCL OTA
